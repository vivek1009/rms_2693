...
locals {
  ...
  vcns = { for key, vcn in local.vcns_map : vcn.name => {
    ...
    defined_tags      = null
    subnets = { for s in local.spoke_subnet_names : replace("snet-${vcn.name}-${s}", "vcn-", "") => {
      #subnets = { for s in local.spoke_subnet_names : replace("snet-${vcn.name}-${s}", "-vcn", "") => {
      compartment_id  = null
      name            = replace("snet-${vcn.name}-${s}", "vcn-", "")
      #name            = replace("snet-${vcn.name}-${s}", "-vcn", "")
      defined_tags    = null
# Edit remy - Subnets en 3 bits au lieu de 4 car VCNs en /21
#      cidr            = cidrsubnet(vcn.cidr, 4, index(local.spoke_subnet_names, s))
      cidr            = cidrsubnet(vcn.cidr, 3, index(local.spoke_subnet_names, s))
      dns_label       = s
      ...
      security_lists = { "security-list" : {
        ...
        ingress_rules : [{
          ...
          protocol : "6"
          stateless : false
          description : "Permet les connections SSH vers les hôtes dans la plage ${vcn.cidr}."
          src : vcn.cidr
          src_type : "CIDR_BLOCK"
          icmp_type : null
          icmp_code : null
  ...
  web_route_tables = { for key, subnet in local.all_lz_spoke_subnets : replace("rttbl-${key}", "vcn-", "") => {
    ...
    route_rules = concat([{
      ...
      description       = "Traffic destiné vers ${local.valid_service_gateway_cidrs[0]} dirigé au Service Gateway."
      },
      {
        is_create         = length(var.dmz_vcn_cidr) == 0 && ! var.no_internet_access ? true : false
        ...
        destination_type  = "SERVICE_CIDR_BLOCK"
        network_entity_id = module.lz_vcn_spokes.service_gateways[subnet.vcn_id].id
        description       = "Traffic destined to ${local.valid_service_gateway_cidrs[1]} dirigé au Service Gateway."
      },
      {
        ...
        description       = "Traffic destiné vers ${local.anywhere} goes to Internet Gateway."
      },
      {
        is_create         = length(var.dmz_vcn_cidr) == 0 && ! var.no_internet_access ? true : false
        destination       = local.anywhere
        ...
        description       = "Traffic destiné vers ${local.anywhere} dirigé au Internet Gateway."
      ...
      [for vcn_name, vcn in local.all_lz_spoke_vcn_ids : {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné vers le VCN ${vcn_name} dirigé au  DRG."
      ...
      [for cidr in var.onprem_cidrs : {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné vers les infrastructures on-premises (${cidr}) dirigé au DRG."
        }
      ]
    )
  ...
  app_route_tables = { for key, subnet in local.all_lz_spoke_subnets : replace("rttbl-${key}", "vcn-", "") => {
    compartment_id = subnet.compartment_id
    vcn_id         = subnet.vcn_id
    subnet_id      = subnet.id
    defined_tags   = null
    route_rules = concat([{
      ...
      destination_type  = "SERVICE_CIDR_BLOCK"
      network_entity_id = module.lz_vcn_spokes.service_gateways[subnet.vcn_id].id
      description       = "Traffic destined to ${local.valid_service_gateway_cidrs[0]} dirigé au Service Gateway."
      },
      {
        is_create         = length(var.dmz_vcn_cidr) > 0
        destination       = local.anywhere
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné vers  ${local.anywhere} goes to DRG."
      },
      {
        is_create         = length(var.dmz_vcn_cidr) == 0 && ! var.no_internet_access ? true : false
        destination       = local.anywhere
        ...
        description       = "Traffic destiné vers ${local.anywhere} goes to NAT Gateway."
      ...
      [for cidr in var.exacs_vcn_cidrs : {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné vers la plage Exadata (${cidr}) dirigé au DRG."
        }
      ]
    )
  ...
  db_route_tables = { for key, subnet in local.all_lz_spoke_subnets : replace("rttbl-${key}", "vcn-", "") => {
    compartment_id = subnet.compartment_id
    vcn_id         = subnet.vcn_id
    subnet_id      = subnet.id
    defined_tags   = null
    route_rules = concat([{
      ...
      },
      {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné vers ${local.anywhere} dirigé au DRG."
      }
      ],
      [for vcn_name, vcn in local.all_lz_spoke_vcn_ids : {
        is_create         = var.hub_spoke_architecture && length(var.dmz_vcn_cidr) == 0
...
#Edit Remy - Ajout subnet gestion
## Gestion Subnet Route Tables
  gestion_route_tables = { for key, subnet in local.all_lz_spoke_subnets : replace("rttbl-${key}", "vcn-", "") => {
    compartment_id = subnet.compartment_id
    vcn_id         = subnet.vcn_id
    subnet_id      = subnet.id
    defined_tags   = null
  ...
  } if length(regexall(".*-${local.spoke_subnet_names[3]}-*", key)) > 0 }
#End edit Remy subnet gestion

#Edit Remy : gestion_route_tables
  lz_subnets_route_tables = merge(local.web_route_tables, local.app_route_tables, local.db_route_tables, local.gestion_route_tables)
