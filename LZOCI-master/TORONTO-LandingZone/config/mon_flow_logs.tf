...
locals {
  ...
  flow_logs = { for k, v in local.all_lz_subnets : k =>
    {
      log_display_name              = "log-${k}-networkflow",
      log_type                      = "SERVICE",
...
module "lz_flow_logs" {
  ...
  log_group_display_name = "loggrp-oct-subnet-flow-logs"
  log_group_description  = "Groupe de logs des subnets de la Landing Zone ${var.service_label}."
