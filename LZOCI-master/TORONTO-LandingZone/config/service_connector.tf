...
module "lz_sch_audit_objStore_policy" {
  ...
  policies = {
    (local.sch_audit_policy_name) = {
      ...
      description            = "Politique de la Landing Zone pour le Service Connector Hub afin de gérer les objets dans le bucket cible."
      statements = [
                    <<EOF
...
module "lz_sch_audit_streaming_policy" {
  ...
  policies = {
    (local.sch_audit_policy_name) = {
      compartment_id         = local.parent_compartment_id
      description            = "Politique de la Landing Zone pour le Service Connector Hub afin de gérer les messages dans le stream."
      statements = [
                    <<EOF
...
module "lz_sch_audit_functions_policy" {
  ...
  policies = {
    (local.sch_audit_policy_name) = {
      compartment_id         = local.parent_compartment_id
      description            = "Politique de la Landing Zone pour le Service Connector Hub afin d'utiliser les fonctions."
      statements = [
                    <<EOF
...
module "lz_sch_vcnFlowLogs_functions_policy" {
  ...
  policies = {
    (local.sch_vcnFlowLogs_policy_name) = {
      compartment_id         = local.parent_compartment_id
      description            = "Politique de la Landing Zone pour le Service Connector Hub afin de d'utiliser les fonctions."
      statements = [
                    <<EOF
