...
locals {
    ...
    subnets = { for s in local.exacs_subnet_names : replace("${vcn.name}-${s}-snt", "-vcn", "") => {
      ...
      security_lists  = {"security-list" = {
        ...
        ingress_rules = [{
          ...
          protocol = "6"
          stateless = false
          description = "Permet les connections SSH à partir des hôtes dans le subnet Exadata Client."
        ...
        egress_rules = [{
          ...
          protocol = "6"
          stateless = false
          description = "Permet les connections SSH cvers les hôtes dans le subnet Exadata Client."
        ...
        },
        {
          ...
          protocol = "6"
          stateless = false
          description = "Permet les connections SSH vers els hôtes dans la plage ${vcn.cidr}."
          dst = vcn.cidr
          dst_type = "CIDR_BLOCK"
          icmp_type = null
          icmp_code = null
  ...
  clt_route_tables = { for key, subnet in module.lz_exacs_vcns.subnets : "${key}-rtable" => {
    ...
    route_rules = concat([{
      ...
      description       = "Traffic destiné à ${local.valid_service_gateway_cidrs[0]} dirigé vers le Service Gateway."
      }],
      [for vcn_name, vcn in module.lz_vcn_dmz.vcns : {
        ...
        description       = "Traffic destiné au VCN ${vcn_name} dirigé au DRG."
      }],
      [for cidr in var.onprem_cidrs : {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné vers les infrastructures on-premises (${cidr}) dirig vers le DRG."
      }],
      [for vcn_name, vcn in local.all_lz_spoke_vcn_ids : {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné au VCN ${vcn_name} dirigé vers le DRG."
  ...
  bkp_route_tables = { for key, subnet in module.lz_exacs_vcns.subnets : "${key}-rtable" => {
    ...
    route_rules = concat([
      ...
      [for cidr in var.onprem_cidrs : {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = var.existing_drg_id != "" ? var.existing_drg_id : (module.lz_drg.drg != null ? module.lz_drg.drg.id : null)
        description       = "Traffic destiné aux infrastructures on-premises (${cidr}) dirigé vers le DRG."
      }],
      [for vcn_name, vcn in local.all_lz_spoke_vcn_ids : {
        is_create         = var.hub_spoke_architecture
        destination       = vcn.cidr_block
