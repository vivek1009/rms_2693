...
locals {
  dmz_bastions_nsg_name   = length(var.dmz_vcn_cidr) > 0 ? "nsg-${local.dmz_vcn_name.name}-bastion" : null
  dmz_services_nsg_name   = length(var.dmz_vcn_cidr) > 0 ? "nsg-${local.dmz_vcn_name.name}-services" : null
  dmz_public_dst_nsg_name = length(var.dmz_vcn_cidr) > 0 ? "nsg-${local.dmz_vcn_name.name}-public-dst" : null

  ssh_dmz_to_spokes_nsg_egress_rules = length(var.dmz_vcn_cidr) > 0 ? { for k, v in module.lz_vcn_spokes.vcns : "${k}-dmz-ssh-egress-rule" => {
    is_create : true,
    description : "Permet les connections SSH aux hôtes dans le VCN ${k} (${v.cidr_block}).",
    protocol : "6",
    stateless : false,
    dst : v.cidr_block,
    dst_type : "CIDR_BLOCK",
  ...
  ssh_dmz_to_exacs_nsg_egress_rules = length(var.dmz_vcn_cidr) > 0 && length(var.exacs_vcn_cidrs) > 0 ? { for k, v in module.lz_exacs_vcns.vcns : "ssh-exacs-${k}-egress-rule" => {
    is_create : true,
    description : "Permet les connections SSH aux hôtes du VCN Exadata ${k} (${v.cidr_block}).",
    protocol : "6",
    stateless : false,
    dst : v.cidr_block,
    dst_type : "CIDR_BLOCK",
  ...
  ons_dmz_to_exacs_nsg_egress_rules = length(var.dmz_vcn_cidr) > 0 && length(var.exacs_vcn_cidrs) > 0 ? { for k, v in module.lz_exacs_vcns.vcns : "ons-exacs-${k}-egress-rule" => {
    is_create : true,
    description : "Permet la communication entre le Oracle Notification Services (ONS) et les hôtes du VCN Exadata ${k} (${v.cidr_block}) pour les Fast Application Notifications (FAN).",
    protocol : "6",
    stateless : false,
    dst : v.cidr_block,
    dst_type : "CIDR_BLOCK",
  ...
  sqlnet_dmz_to_exacs_nsg_egress_rules = length(var.dmz_vcn_cidr) > 0 && length(var.exacs_vcn_cidrs) > 0 ? { for k, v in module.lz_exacs_vcns.vcns : "sqlnet-exacs-${k}-egress-rule" => {
    is_create : true,
    description : "Permet les connections SQLNet aux hôtes du VCN Exadata ${k} (${v.cidr_block}).",
    protocol : "6",
    stateless : false,
    dst : v.cidr_block,
    dst_type : "CIDR_BLOCK",
  ...
  http_dmz_to_spokes_nsg_egress_rules = length(var.dmz_vcn_cidr) > 0 ? { for k, v in module.lz_vcn_spokes.vcns : "${k}-dmz-http-egress-rule" => {
    is_create : true,
    description : "Permet les connections HTTP vers le VCN ${k} (${v.cidr_block}).",
    protocol : "6",
    stateless : false,
    dst : v.cidr_block,
    dst_type : "CIDR_BLOCK",
  ...
  public_dst_cidrs_nsg = length(var.public_dst_cidrs) > 0 && length(var.dmz_vcn_cidr) > 0 ? { (local.dmz_public_dst_nsg_name) : {
    ...
    egress_rules : { for cidr in var.public_dst_cidrs : "https-public-dst-egress-rule-${index(var.public_dst_cidrs, cidr)}" => {
      ...
      description : "Permet les connections HTTPS aux hôtes dans la plage ${cidr}.",
      stateless : false,
      protocol : "6",
      dst      = cidr,
      dst_type = "CIDR_BLOCK",
  ...
  nsgs = merge(local.public_dst_cidrs_nsg, {
    (local.dmz_bastions_nsg_name) : {
      ...
      ingress_rules : merge(
        { for cidr in var.public_src_bastion_cidrs : "ssh-public-ingress-rule-${index(var.public_src_bastion_cidrs, cidr)}" => {
          ...
          description : "Permet les connections SSH aux hôtes dans la plage ${cidr}.",
          protocol : "6",
          stateless : false,
          src : cidr,
          src_type : "CIDR_BLOCK",
        ...
        { for cidr in var.onprem_src_ssh_cidrs : "ssh-onprem-ingress-rule-${index(var.onprem_src_ssh_cidrs, cidr)}" => {
          ...
          description : "Permet les connections SSH aux hôtes de la plage on-premises (${cidr}).",
          protocol : "6",
          stateless : false,
          src : cidr,
          src_type : "CIDR_BLOCK",
      ...
      egress_rules : merge(local.ssh_dmz_to_spokes_nsg_egress_rules, local.ssh_dmz_to_exacs_nsg_egress_rules,
        { dmz-services-egress-rule : {
          is_create : true,
          description : "Permet les connections SSH aux hôtes dans le NSG ${local.dmz_services_nsg_name}.",
          protocol : "6",
          stateless : false,
        ...
        { osn-services-egress-rule : {
          is_create : true,
          description : "Permet les connections HTTPS aux hôtes dans la plage du Service Gateway ${local.valid_service_gateway_cidrs[0]}.",
          protocol : "6",
          stateless : false,
    ...
    (local.dmz_services_nsg_name) : {
      ...
      ingress_rules : merge({
        ssh-ingress-rule : {
          is_create : true,
          description : "Permet les connections SSH aux hôtes dans le NSG ${local.dmz_bastions_nsg_name}.",
          protocol : "6",
          stateless : false,
          ...
          description : "Permet les connections HTTPS aux hôtes dans la plage on-premises ${cidr}.",
          protocol : "6",
          stateless : false,
          src : cidr,
          src_type : "CIDR_BLOCK",
      ...
      egress_rules : merge(local.http_dmz_to_spokes_nsg_egress_rules, local.ons_dmz_to_exacs_nsg_egress_rules, local.sqlnet_dmz_to_exacs_nsg_egress_rules,
        { osn-services-egress-rule : {
          is_create : true,
          description : "Permet les connections HTTPS vers la plage du Service Gateway (${local.valid_service_gateway_cidrs[0]}).",
          protocol : "6",
          stateless : false,
          dst      = local.valid_service_gateway_cidrs[0],
          dst_type = "SERVICE_CIDR_BLOCK",
