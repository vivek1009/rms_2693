...
module "lz_scanning" {
  ...
  scan_targets = var.vss_create == true ? {
    (local.security_cmp_target_name) = {
      compartment_id        = module.lz_compartments.compartments[local.security_compartment.key].id
      description           = "Landing Zone : Cible de scan du compartiment ${local.security_compartment.name}."
    ...
    (local.network_cmp_target_name) = {
      compartment_id        = module.lz_compartments.compartments[local.security_compartment.key].id
      description           = "Landing Zone : Cible de scan du compartiment ${local.network_compartment.name}."
      scan_recipe_name      = local.scan_default_recipe_name
    ...
    (local.appdev_cmp_target_name) = {
      compartment_id        = module.lz_compartments.compartments[local.security_compartment.key].id
      description           = "Landing Zone : Cible de scan du compartiment ${local.appdev_compartment.name}."
      scan_recipe_name      = local.scan_default_recipe_name
    ...
    (local.database_cmp_target_name) = {
      compartment_id        = module.lz_compartments.compartments[local.security_compartment.key].id
      description           = "Landing Zone : Cible de scan du compartiment ${local.database_compartment.name}."
      scan_recipe_name      = local.scan_default_recipe_name
      target_compartment_id = module.lz_compartments.compartments[local.database_compartment.key].id
      defined_tags          = null
    }, #Edit Remy - ajout preprod
    (local.appdevpreprod_cmp_target_name) = {
      compartment_id        = module.lz_compartments.compartments[local.security_compartment.key].id
      description           = "Landing Zone : Cible de scan du compartiment ${local.appdevpreprod_compartment.name}."
      scan_recipe_name      = local.scan_default_recipe_name
      target_compartment_id = module.lz_compartments.compartments[local.appdevpreprod_compartment.key].id
      defined_tags          = null
    },
    (local.databasepreprod_cmp_target_name) = {
      compartment_id        = module.lz_compartments.compartments[local.security_compartment.key].id
      description           = "Landing Zone : Cible de scan du compartiment ${local.databasepreprod_compartment.name}."
      scan_recipe_name      = local.scan_default_recipe_name
      target_compartment_id = module.lz_compartments.compartments[local.databasepreprod_compartment.key].id
      defined_tags          = null
    } # Fin edit Remy
  } : {}
}
