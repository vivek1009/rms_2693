...
module "lz_notifications" {
  ...
  rules = {
    ("ntf-${var.service_label}-notify-on-iam-changes") = {
      compartment_id      = var.tenancy_ocid
      description         = "Règle d'événements de la Landing Zone qui détecte les modifications aux ressources IAM (création, modification, supression)."
      is_enabled          = true
      condition           = <<EOT
            {"eventType": 
      ...
      actions_description = "Envoie une notification via ONS"
      topic_id            = module.lz_security_topic.topic.id
      defined_tags        = null
    },
    ("ntf-${var.service_label}-notify-on-network-changes") = {
      compartment_id      = local.parent_compartment_id
      description         = "Règle d'événements de la Landing Zone qui détecte les modifications aux ressources réseau (création, modification, supression)."
      is_enabled          = true
      condition           = <<EOT
        {"eventType":
